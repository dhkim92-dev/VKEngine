...
int main(int argc, const char *argv[])	
{
	...
	unsigned char* img_data = loadImageFile("assets/image/lena.jpeg", 1200, 822, 3);
	...
	vector<VkDescriptorPoolSize> pool_sizes = {
		infos::descriptorPoolSize(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 2),
	...
	VK_CHECK_RESULT(d_input.alloc( 1200*822*3, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT ));
	VK_CHECK_RESULT(d_output.alloc( 1200*822*3, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT ));
	...
	*/
	...
	unsigned char *cvt_data = new unsigned char[1200*822*3];
	h_output.copyTo(conv_data, 1200*822*3);
	//h_output.destroy();
	for(uint32_t i = 0 ; i < 1200*822*3 ; i++){
		if(conv_data[i]== 0)
			printf("%.1f \n", conv_data[i]);
	}

	//stbi_write_jpg("test.jpeg", 1200, 822, 0, cvt_data,100);)
